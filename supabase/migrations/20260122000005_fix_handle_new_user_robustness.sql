-- Fix handle_new_user trigger to handle errors gracefully and ensure correct data insertion
...
BEGIN
  -- Extract and normalize RUT from metadata
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Log error or ignore to prevent auth signup failure
    RAISE WARNING 'Error in handle_new_user linking: %', SQLERRM;
  END;

  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- CRITICAL: Prevent the entire signup from failing if public.users insertion fails
  RAISE WARNING 'Error in handle_new_user main: %', SQLERRM;
  RETURN NEW;
END;
