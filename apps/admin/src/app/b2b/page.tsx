  ...
  return (
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter leading-none text-slate-950">
            Gestión <span className="text-[#1d504b]">B2B</span>
          </h1>
          <p className="text-slate-700 mt-2 font-black uppercase text-xs tracking-widest">Administra solicitudes de empresas, límites de crédito y acceso API.</p>
        </div>
      </div>

        <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-2xl shadow-sm border-2 border-slate-200">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-600" />
          <Input 
            placeholder="Buscar por empresa, RUT o contacto..." 
            className="pl-10 h-12 border-2 border-slate-200 bg-slate-50 focus:bg-white transition-all text-slate-950 font-medium placeholder:text-slate-500"
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 gap-6">
        {isLoading ? (
          <div className="py-20 text-center">
            <div className="animate-spin h-8 w-8 border-4 border-calmar-primary border-t-transparent rounded-full mx-auto mb-4"></div>
