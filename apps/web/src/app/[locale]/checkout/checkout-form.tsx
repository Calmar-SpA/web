    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold italic">{t("emptyCart.title")}</h1>
          <p className="text-slate-500 mt-2">{t("emptyCart.description")}</p>
        </div>
        <Link href="/shop">
          <Button className="bg-slate-900 hover:bg-calmar-ocean text-white font-black italic px-8">
            {t("emptyCart.button")}
          </Button>
        </Link>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-4">
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-black italic tracking-tighter mt-4 uppercase">{t("title")}</h1>
      </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          <section className="space-y-6">
            <div className="flex items-center gap-2 border-b pb-4 border-slate-100">
              ...
              <h2 className="text-xl font-bold italic uppercase tracking-tight">{t("shipping.title")}</h2>
            </div>
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 border-b pb-4 border-slate-100">
              ...
              <h2 className="text-xl font-bold italic uppercase tracking-tight">{t("payment.title")}</h2>
            </div>
        ...
        <div className="lg:col-span-5">
          <Card className="sticky top-32 border-calmar-ocean/10 shadow-2xl shadow-slate-200/50 overflow-hidden">
            <CardHeader className="bg-slate-900 border-b border-white/10 text-white">
              <CardTitle className="text-lg italic uppercase tracking-tight">{t("summary.title")}</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-6">
              <div className="max-h-[300px] overflow-y-auto space-y-4 pr-2">
                {items.map((item) => (
                  <div key={item.product.id} className="flex gap-4">
                    <div className="w-16 h-16 bg-slate-50 rounded-lg flex-shrink-0 flex items-center justify-center p-2 border border-slate-100">
                      <div className="text-slate-300 text-xs text-center">IMG</div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm truncate">{item.product.name}</p>
                      <p className="text-xs text-slate-500">{t("summary.quantity", { qty: item.quantity })}</p>
                      <p className="font-black italic text-calmar-ocean text-sm mt-1">
              ...
              <div className="space-y-3 pt-6 border-t border-slate-100">
                ...
                {b2bDiscount > 0 && (
                  <div className="flex justify-between text-sm text-calmar-ocean-dark font-black italic uppercase tracking-tighter">
                ...
                <div className="flex justify-between text-2xl pt-4 border-t border-slate-900 uppercase">
                  <span className="font-black italic tracking-tighter">{t("summary.total")}</span>
                  <span className="font-black italic text-slate-900">${finalTotal.toLocaleString('es-CL')}</span>
                </div>
              </div>
              ...
              <Button 
                ...
                className="w-full h-16 bg-slate-900 hover:bg-calmar-ocean text-white text-xl font-black italic shadow-xl shadow-slate-900/20 transition-all hover:translate-y-[-2px] active:translate-y-0 disabled:opacity-50 disabled:translate-y-0"
              >
